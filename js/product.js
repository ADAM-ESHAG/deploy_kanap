...
fetch(`https://kanap-9guc.onrender.com/api/products/${productId}`).then((response) =>
  response.json().then((data) => {
    ...
    btnAddProduct.addEventListener("click", () => {
      ...
      let imageItemValueSrc = `${data.imageUrl}`; // Récupération d'image
      let imageItemValueAlt = `${data.altTxt}`;
      ...
      //la couleur et la quantité sont valides, alors
        ...
        let choiceProduct = {
          ...
          imageOfProduct: imageItemValueSrc,
          imageAltOfProduct : imageItemValueAlt,
